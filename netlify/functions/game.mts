...
export default async (req: Request, context: Context) => {
  ...
  try {
    ...
    const url = `https://www.giantbomb.com/api/game/${guid}?${params.toString()}`;

    console.log("url: ", url);
    
    // send req to giant bomb api
    const response = await fetch(`https://www.giantbomb.com/api/game/${guid}?${params.toString()}`, {
      method: 'GET',
    });
